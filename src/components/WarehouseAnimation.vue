  ...
  <div class="scene-wrapper">
    ...
    >
      ...
      <rect
        ...
        height="10"
        rx="2"
        :fill="COLORS.conveyor"
      />
      <line
        v-for="rx in convRollers"
        :key="rx"
        :x1="rx"
        y1="130"
        :x2="rx"
        y2="140"
        :stroke="COLORS.conveyorRoller"
        stroke-width="2"
      />
      ...
      <g clip-path="url(#warehouseConvClip)">
        <g v-for="box in conveyorBoxes" :key="box.id">
          <rect
            ...
            :height="SCENE.boxH"
            rx="2"
            :fill="box.type === 'A' ? COLORS.boxA : COLORS.boxB"
          />
          <rect
            v-if="box.type === 'E'"
            :x="box.x"
            :y="SCENE.convBoxY + SCENE.boxH / 2"
            :width="box.width"
            :height="SCENE.boxH / 2"
            rx="2"
            :fill="COLORS.boxE"
          ...
          <template v-if="box.type === 'EB'">
            ...
            <line
              :x1="box.x"
              :y1="SCENE.convBoxY + SCENE.boxH / 2"
              :x2="box.x + box.width"
              :y2="SCENE.convBoxY + SCENE.boxH / 2"
              :stroke="COLORS.boxELine"
              stroke-width="1"
            />
          </template>
        </g>
      ...
      <g v-for="(box, i) in bufferSlots" :key="'buf-box-' + i">
        <template v-if="box !== null">
          <rect
            v-if="box.type === 'A' || box.type === 'B'"
            :x="SCENE.bufferSlotX[i] - box.width / 2"
            :y="SCENE.bufferY"
            :width="box.width"
            :height="SCENE.boxH"
            rx="2"
          ...
          <template v-if="box.type === 'EB'">
            ...
            />
            <line
              :x1="SCENE.bufferSlotX[i] - box.width / 2"
              :y1="SCENE.bufferY + SCENE.boxH / 2"
              :x2="SCENE.bufferSlotX[i] + box.width / 2"
              :y2="SCENE.bufferY + SCENE.boxH / 2"
              :stroke="COLORS.boxELine"
              stroke-width="1"
            />
          </template>
      ...
      <g :transform="`translate(${pickPlace.x}, 0)`">
        ...
        <rect
          v-if="
            pickPlace.carriedBox &&
            (pickPlace.carriedBox.type === 'A' ||
              pickPlace.carriedBox.type === 'B')
          "
          :x="-pickPlace.carriedBox.width / 2"
          :y="33 + pickPlace.armLength + 2"
          :width="pickPlace.carriedBox.width"
          :height="SCENE.boxH"
          rx="2"
          :fill="pickPlace.carriedBox.type === 'A' ? COLORS.boxA : COLORS.boxB"
        />
        ...
        <!-- Caisse tenue EB -->
        <template
          v-if="pickPlace.carriedBox && pickPlace.carriedBox.type === 'EB'"
        >
          <rect
            :x="-pickPlace.carriedBox.width / 2"
      ...
      <!-- ===== PALETTE ===== -->
      <!-- Un seul groupe : leaveX=0 en position statique, >0 pendant la sortie/le retour -->
      <g :transform="`translate(${palette.leaveX}, 0)`">
        <rect
          x="560"
          y="152"
          width="114"
        ...
        <g v-for="box in palette.boxes" :key="box.id">
          ...
          <template v-if="box.type === 'EB'">
            ...
            <line
              :x1="box.x"
              :y1="box.y + SCENE.boxH / 2"
              :x2="box.x + box.width"
              :y2="box.y + SCENE.boxH / 2"
              :stroke="COLORS.boxELine"
              stroke-width="1"
            />
          </template>
      ...
      <!-- ===== COMPTEUR DE PALETTES TERMINÉES ===== -->
      <text
        x="617"
        y="176"
        text-anchor="middle"
...
function armLengthFor(yDessus) {
  return yDessus - ARM_BASE_Y - 2;
}
...
const SCENE = {
  ...
  palSlots: 4,
  palLeaveSpeed: 120,
  // Palette (bord gauche x=560, largeur 114) : il faut leaveX > 800-560=240 pour être
  // entièrement hors du viewBox (800 de large). Marge de sécurité incluse.
  palLeaveDistance: 280,
};
...
class PickPlace {
  ...
  /**
   * Saisie depuis tampon : dessus caisse = bufferY = 120 (A/B/EB),
   * ou bufferY + boxH/2 = 130 pour une E seule (demi-hauteur).
   */
  setPickupTargetBuffer(bufIdx, type = "A") {
    this._pickupX = SCENE.bufferSlotX[bufIdx];
    const yTop = type === "E" ? SCENE.bufferY + SCENE.boxH / 2 : SCENE.bufferY;
    this._pickupArmLength = armLengthFor(yTop);
  }
  ...
  /**
   * Stack E2 sur E1. `baseYTop` = dessus de la caisse E1 (base) avant empilement :
   * convoyeur → convBoxY + boxH/2 = 120 ; tampon → bufferY + boxH/2 = 130.
   */
  setStackTarget(targetBox, baseYTop = SCENE.convBoxY + SCENE.boxH / 2) {
    this._depositX = targetBox.x + targetBox.width / 2;
    this._depositArmLength = armLengthFor(baseYTop);
  }

  get gripHalfW() {
    return Math.floor(BOX_WIDTHS.A / 2) + 1;
  }
}
...
class Palette {
  constructor() {
    this.boxes = [];
    this.leaving = false; // en cours de sortie (pleine) vers la droite
    this.entering = false; // en cours de retour (vide) depuis la droite
    this.leaveX = 0; // décalage horizontal appliqué au groupe palette
  ...
  get isFull() {
    return this._grid.every((r) => r.every((c) => c !== null));
  }

  /** Vide le contenu (fin de la sortie, palette hors écran) et grille remise à zéro. */
  empty() {
    this.boxes = [];
    this._grid = Array.from({ length: 5 }, () => [null, null, null, null]);
  }

  /** Fin du retour : la palette vide est de nouveau à sa place, prête à recevoir. */
  arrive() {
    this.leaving = false;
    this.entering = false;
    this.leaveX = 0;
...
const completedPalettes = ref(0); // Nb de palettes complètes depuis le chargement de la page (non persisté)
...
let pendingStack = null; // { base: {source,index}, top: {source,index} } pour un empilement E en cours

// ─── Randomizer ───────────────────────────────────────────────────────────────
// Les caisses E ne sont plus forcées à arriver par paire consécutive : elles
// apparaissent indépendamment, et une E isolée patiente en zone tampon en
// attendant sa complémentaire (cf. findStackPair).
let consecutiveB = 0;
...
function tick(ts) {
  ...
  // 4. Palette qui part, se vide hors écran, puis revient (même trajet, inversé)
  if (palette.leaving) {
    palette.leaveX += SCENE.palLeaveSpeed * dt;
    if (palette.leaveX >= SCENE.palLeaveDistance) {
      palette.leaveX = SCENE.palLeaveDistance;
      palette.empty(); // vidée pendant qu'elle est hors écran
      palette.leaving = false;
      palette.entering = true; // amorce le retour, toujours hors écran
    }
  } else if (palette.entering) {
    palette.leaveX -= SCENE.palLeaveSpeed * dt;
    if (palette.leaveX <= 0) {
      palette.arrive(); // de retour à sa place, vide et prête
    }
  }
...
/**
 * Cherche une paire de caisses E empilables, tous emplacements confondus
 * (convoyeur en tête, zone tampon). La "base" reste en place et devient EB ;
 * la "top" est déplacée par-dessus. Ordre de préférence :
 *   1. Deux E arrêtées en tête de convoyeur (cas historique, le plus rapide)
 *   2. Une E en tampon (base) + une E arrêtée en tête de convoyeur (top)
 *   3. Deux E en tampon
 * Retourne { base: {source,index,box}, top: {source,index,box} } ou null.
 */
function findStackPair() {
  const boxes = conveyorBoxes.value;
  const convE0 = boxes[0]?.type === "E" && isStopped(0);
  const convE1 = boxes[1]?.type === "E" && isStopped(1);

  if (convE0 && convE1) {
    return {
      base: { source: "conveyor", index: 0, box: boxes[0] },
      top: { source: "conveyor", index: 1, box: boxes[1] },
    };
  }

  const bufEIdx = bufferSlots.findIndex((b) => b?.type === "E");

  if (bufEIdx !== -1 && convE0) {
    return {
      base: { source: "buffer", index: bufEIdx, box: bufferSlots[bufEIdx] },
      top: { source: "conveyor", index: 0, box: boxes[0] },
    };
  }

  if (bufEIdx !== -1) {
    const secondBufEIdx = bufferSlots.findIndex(
      (b, i) => i !== bufEIdx && b?.type === "E"
    );
    if (secondBufEIdx !== -1) {
      return {
        base: { source: "buffer", index: bufEIdx, box: bufferSlots[bufEIdx] },
...
function chooseCaisse() {
  // Palette absente de son emplacement (sortie ou pas encore revenue) → pas de dépôt possible
  if (palette.leaving || palette.entering) return null;
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (!box) continue;
    if (box.type === "E") continue; // E seule → doit être empilée d'abord
  ...
  for (let i = 0; i < Math.min(boxes.length, 2); i++) {
    ...
    if (slot)
      candidates.push({ source: "conveyor", index: i, box: boxes[i], slot });
  }
 ...
 * mais qu'on peut mettre en tampon (libérer le convoyeur). Une caisse E isolée
 * (pas de paire à empiler dans l'immédiat) y patiente aussi, en attendant sa
 * complémentaire — c'est ce qui casse la consécutivité systématique des E.
...
function chooseCaisseForBuffer() {
  ...
  for (let i = 0; i < Math.min(boxes.length, 2); i++) {
    ...
    if (boxes[i].type !== "E" && palette.nextSlot(boxes[i].type)) continue;
...
function updatePickPlace(dt) {
  ...
  switch (pp.state) {
    case "idle": {
      // Priorité 1 : empiler une paire E (base en place, top déplacée dessus)
      const pair = findStackPair();
      if (pair) {
        pendingStack = pair;

        if (pair.top.source === "buffer")
          pp.setPickupTargetBuffer(pair.top.index, "E");
        else pp.setPickupTargetE(pair.top.box);

        const baseYTop =
          pair.base.source === "buffer"
            ? SCENE.bufferY + SCENE.boxH / 2
            : SCENE.convBoxY + SCENE.boxH / 2;
        pp.setStackTarget(pair.base.box, baseYTop);
    ...
    case "going-stack-pickup": {
      if (moveX(pp, pp._pickupX, dt)) {
        startMove();
        pp.state = "stack-picking";
      }
      break;
    }
    case "stack-picking": {
      if (moveArm(pp, pp._pickupArmLength, dt)) {
        // Retire la caisse "top" de sa source (convoyeur ou tampon)
        let box;
        if (pendingStack.top.source === "buffer") {
          box = bufferSlots[pendingStack.top.index];
          bufferSlots[pendingStack.top.index] = null;
        } else {
          box = conveyorBoxes.value.splice(pendingStack.top.index, 1)[0];
        }
    ...
    case "stack-retracting-pickup": {
      if (moveArm(pp, SCENE.armBase, dt)) {
        startMove();
        pp.state = "going-stack-deposit";
      }
      break;
    }
    case "going-stack-deposit": {
      if (moveX(pp, pp._depositX, dt)) {
        startMove();
        pp.state = "stack-depositing";
      }
      break;
    }
    case "stack-depositing": {
      if (moveArm(pp, pp._depositArmLength, dt)) {
        // Fusionne "top" sur "base" : la base devient EB, là où elle se trouve
        const { base } = pendingStack;
        if (pp.carriedBox) {
          const eb = new Box(base.box.id, "EB");
          eb.x = base.box.x;
          if (base.source === "buffer") bufferSlots[base.index] = eb;
          else conveyorBoxes.value.splice(base.index, 1, eb);
        }
        pp.carriedBox = null;
        pendingStack = null;
        startMove();
    ...
    case "going-deposit-buffer": {
      if (moveX(pp, pp._depositX, dt)) {
        startMove();
        pp.state = "depositing-buffer";
      }
      break;
    }
    case "depositing-buffer": {
      if (moveArm(pp, pp._depositArmLength, dt)) {
        if (pp.carriedBox && pendingBufIdx >= 0)
          bufferSlots[pendingBufIdx] = pp.carriedBox;
        pp.carriedBox = null;
        pendingBufIdx = -1;
        startMove();
    ...
    case "depositing": {
      if (moveArm(pp, pp._depositArmLength, dt)) {
        if (pp.carriedBox && pendingSlot)
          palette.deposit(pp.carriedBox, pendingSlot);
        pp.carriedBox = null;
        pendingSlot = null;
        pendingBufIdx = -1;
        startMove();
    ...
    case "retracting-deposit": {
      if (moveArm(pp, SCENE.armBase, dt)) {
        startMove();
        if (palette.isFull) {
          palette.leaving = true;
          completedPalettes.value++;
          pp.state = "idle";
