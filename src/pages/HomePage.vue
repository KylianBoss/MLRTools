          ...
          <div class="row items-start q-gutter-md">
            ...
            <div class="col">
              ...
              <p class="text-dark q-mb-md">
                Cette instance de l'application tourne en mode automatique pour
                la récupération des données.<br />
                Vous n'avez aucune interaction possible depuis cette instance et
                ce compte.
              </p>
            </div>
          </div>
        ...
        <q-card-section v-if="App.cronJobsInitialized">
          <div class="row items-start q-gutter-md">
            ...
            <div class="col">
              ...
              <p class="text-dark q-mb-md">
                Les tâches planifiées pour la récupération des données sont en
                cours d'exécution.
              </p>
              ...
              <ul>
                <li v-for="job in cronJobs" :key="job.name">
                  ...
                  <q-chip
                    :color="
                      job.actualState === 'running'
                        ? 'green'
                        : job.actualState === 'error'
                        ? 'red'
                        : 'grey'
                    "
                    text-color="white"
                  ...
                  Dernier log:
                  {{ job.lastLog ? job.lastLog : "Aucun log disponible" }}<br />
                </li>
              </ul>
            </div>
          </div>
        ...
        <q-card-section>
          <div class="row items-start q-gutter-md">
            ...
            <div class="col">
              <h5 class="q-mt-none q-mb-sm">Bienvenue sur MLR Tools</h5>
              <p class="text-dark q-mb-md">
                Cette application vous permet de suivre et d'analyser les
                données de production.<br />
          ...
          <div class="row items-start q-gutter-md">
            ...
            <div class="col">
              <h5 class="q-mt-none q-mb-sm">
                Plus de prise en compte de certaines alarmes
              </h5>
              <div class="text-dark q-mb-md">
                Les alarmes suivantes ne sont plus prises en compte dans le
                calcul et ne sont plus enregistrées dans la base de données :
                <ul>
                  <li>
                    Warning | M6009.0306 | Shuttle X - Level X ~ Élévateur de
                    shuttles en WarmUp
                  </li>
                  <li>
                    Warning | M6130.0201 | TLXX - Impossible de démarrer le
                    WarmUp
                  </li>
                  <li>Warning | M6130.0202 | TLXX - WarmUp actif</li>
                  <li>
                    Warning | M6130.0203 | TLXX - Données cinématiques de WarmUp
                    activées
                  </li>
                </ul>
              </div>
...
const fetchCronStatus = async () => {
  ...
  eventSource.onmessage = (event) => {
    ...
    if (data.type === "cronJobStatus") {
      const index = cronJobs.value.findIndex(
        (job) => job.action === data.job.action
      );
      if (index !== -1) {
    ...
    if (App.isBot) {
      ...
      setInterval(async () => {
        ...
        await api.get("/bot/needs-restart/" + App.userId).then(async (res) => {
          ...
          await api.post("/bot/restart-ack/" + App.userId).then(() => {
